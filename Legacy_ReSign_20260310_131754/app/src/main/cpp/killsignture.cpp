...
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
#include "bytehook.h"
#ifdef __cplusplus
extern "C" {
#endif
__attribute__((visibility("default"))) int shadowhook_register_dl_init_callback(void (*pre)(const char*, void*), void* data) { (void)pre; (void)data; return 0; }
__attribute__((visibility("default"))) int shadowhook_register_dl_fini_callback(void (*post)(const char*, int, void*), void* data) { (void)post; (void)data; return 0; }
#ifdef __cplusplus
}
#endif
#endif
...
static CloseFn g_orig_close = nullptr;
...
static volatile int g_plt_inited = 0;

static void resolve_orig_fns_once() {
  static int inited = 0;
  if (__atomic_exchange_n(&inited, 1, __ATOMIC_ACQ_REL) != 0) return;

  g_orig_open = (OpenFn)dlsym(RTLD_NEXT, "open");
  g_orig_openat = (OpenatFn)dlsym(RTLD_NEXT, "openat");
  g_orig_openat2 = (Openat2Fn)dlsym(RTLD_NEXT, "openat2");
  g_orig_close = (CloseFn)dlsym(RTLD_NEXT, "close");
  g_orig_readlink = (ReadlinkFn)dlsym(RTLD_NEXT, "readlink");
  g_orig_readlinkat = (ReadlinkatFn)dlsym(RTLD_NEXT, "readlinkat");
  g_orig_access = (AccessFn)dlsym(RTLD_NEXT, "access");
  g_orig_faccessat = (FaccessatFn)dlsym(RTLD_NEXT, "faccessat");
  g_orig_stat = (StatFn)dlsym(RTLD_NEXT, "stat");
  g_orig_lstat = (LstatFn)dlsym(RTLD_NEXT, "lstat");
  g_orig_newfstatat = (NewfstatatFn)dlsym(RTLD_NEXT, "newfstatat");
  g_orig_statx = (StatxFn)dlsym(RTLD_NEXT, "statx");
}

static void install_plt_backend_best_effort() {
#if !defined(RESIG_HAS_BYTEHOOK) || !(RESIG_HAS_BYTEHOOK)
  return;
#else
  int expected = 0;
  if (!__atomic_compare_exchange_n(&g_plt_inited, &expected, 1, false, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED)) return;

  resolve_orig_fns_once();
  (void)bytehook_init(BYTEHOOK_MODE_AUTOMATIC, 0);

  (void)bytehook_hook_all(nullptr, "open", (void*)open_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "openat", (void*)openat_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "openat2", (void*)openat2_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "close", (void*)close_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "__close", (void*)close_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "readlink", (void*)readlink_hook, nullptr, nullptr);
  (void)bytehook_hook_all(nullptr, "readlinkat", (void*)readlinkat_hook, nullptr, nullptr);

  if (__atomic_load_n(&g_seccomp_full_enabled, __ATOMIC_RELAXED) != 0) {
    (void)bytehook_hook_all(nullptr, "faccessat", (void*)faccessat_hook, nullptr, nullptr);
    (void)bytehook_hook_all(nullptr, "stat", (void*)stat_hook, nullptr, nullptr);
    (void)bytehook_hook_all(nullptr, "lstat", (void*)lstat_hook, nullptr, nullptr);
    (void)bytehook_hook_all(nullptr, "newfstatat", (void*)newfstatat_hook, nullptr, nullptr);
    (void)bytehook_hook_all(nullptr, "statx", (void*)statx_hook, nullptr, nullptr);
  }

  g_redirect_backend = REDIRECT_BACKEND_PLT;
#endif
}
...
static int open_hook(const char *pathname, int flags, ...) {
  ...
  if (flags & O_CREAT) {
    fd =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(open_hook, p, flags, mode);
#else
        (g_orig_open ? g_orig_open(p, flags, mode) : -1);
#endif
  } else {
    fd =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(open_hook, p, flags);
#else
        (g_orig_open ? g_orig_open(p, flags) : -1);
#endif
  }
...
static int openat_hook(int dirfd, const char *pathname, int flags, ...) {
  ...
  if (flags & O_CREAT) {
    fd =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(openat_hook, dirfd, p, flags, mode);
#else
        (g_orig_openat ? g_orig_openat(dirfd, p, flags, mode) : -1);
#endif
  } else {
    fd =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(openat_hook, dirfd, p, flags);
#else
        (g_orig_openat ? g_orig_openat(dirfd, p, flags) : -1);
#endif
  }
...
static int openat2_hook(int dirfd, const char* pathname, const void* how, size_t size) {
  ...
  int fd =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(openat2_hook, dirfd, p, how, size);
#else
      (g_orig_openat2 ? g_orig_openat2(dirfd, p, how, size) : -1);
#endif
  if (fd >= 0 && (did_redirect || (p && g_rep_path && c_ends_with(p, g_rep_path)))) {
    mark_redirected_fd(fd);
  }
...
static int close_hook(int fd) {
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(close_hook, fd);
#else
      (g_orig_close ? g_orig_close(fd) : -1);
#endif
  int saved_errno = errno;
  if (r == 0 || saved_errno == EINTR || saved_errno == EBADF) {
...
static int access_hook(const char* pathname, int mode) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(access_hook, p, mode);
#else
      (g_orig_access ? g_orig_access(p, mode) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int faccessat_hook(int dirfd, const char* pathname, int mode, int flags) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(faccessat_hook, dirfd, p, mode, flags);
#else
      (g_orig_faccessat ? g_orig_faccessat(dirfd, p, mode, flags) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int stat_hook(const char* pathname, struct stat* st) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(stat_hook, p, st);
#else
      (g_orig_stat ? g_orig_stat(p, st) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int lstat_hook(const char* pathname, struct stat* st) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(lstat_hook, p, st);
#else
      (g_orig_lstat ? g_orig_lstat(p, st) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int newfstatat_hook(int dirfd, const char* pathname, struct stat* st, int flags) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(newfstatat_hook, dirfd, p, st, flags);
#else
      (g_orig_newfstatat ? g_orig_newfstatat(dirfd, p, st, flags) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int statx_hook(int dirfd, const char* pathname, int flags, unsigned int mask, void* buf) {
  ...
  int r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(statx_hook, dirfd, p, flags, mask, buf);
#else
      (g_orig_statx ? g_orig_statx(dirfd, p, flags, mask, buf) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static ssize_t readlink_hook(const char *pathname, char *buf, size_t bufsiz) {
  if (pathname && g_apk_path && c_has_sub(pathname, "origin.apk")) {
    ssize_t rr = fake_readlink_to_src(buf, bufsiz);
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
    BYTEHOOK_POP_STACK();
#endif
    return rr;
  ...
  if (should_fake_readlink_for_fd_path(pathname, &fd) && fd >= 0 && is_redirected_fd(fd) && g_apk_path) {
    ...
    ssize_t r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(readlink_hook, pathname, tmp, sizeof(tmp) - 1);
#else
        (g_orig_readlink ? g_orig_readlink(pathname, tmp, sizeof(tmp) - 1) : -1);
#endif
    if (r > 0) {
  ...
  ssize_t out =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(readlink_hook, pathname, buf, bufsiz);
#else
      (g_orig_readlink ? g_orig_readlink(pathname, buf, bufsiz) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static ssize_t readlinkat_hook(int dirfd, const char *pathname, char *buf, size_t bufsiz) {
  ...
  if (should_fake_readlink_for_fd_path(pathname, &fd) && fd >= 0 && is_redirected_fd(fd) && g_apk_path) {
    ...
    ssize_t r =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
        BYTEHOOK_CALL_PREV(readlinkat_hook, dirfd, pathname, tmp, sizeof(tmp) - 1);
#else
        (g_orig_readlinkat ? g_orig_readlinkat(dirfd, pathname, tmp, sizeof(tmp) - 1) : -1);
#endif
    if (r > 0) {
      tmp[(r < (ssize_t)sizeof(tmp) - 1) ? r : (ssize_t)sizeof(tmp) - 1] = '\0';
      int match = 0;
  ...
  ssize_t out =
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
      BYTEHOOK_CALL_PREV(readlinkat_hook, dirfd, pathname, buf, bufsiz);
#else
      (g_orig_readlinkat ? g_orig_readlinkat(dirfd, pathname, buf, bufsiz) : -1);
#endif
#if defined(RESIG_HAS_BYTEHOOK) && RESIG_HAS_BYTEHOOK
  BYTEHOOK_POP_STACK();
#endif
...
static int install_seccomp_filter_all_threads(uintptr_t allow1_start,
                                              uintptr_t allow1_end_inclusive,
                                              uintptr_t allow2_start,
                                              uintptr_t allow2_end_inclusive) {
  if (allow1_start == 0 || allow1_end_inclusive == 0 || allow1_end_inclusive < allow1_start) {
  ...
  const uint64_t a1s = (uint64_t)allow1_start;
  const uint64_t a1e = (uint64_t)allow1_end_inclusive;
  const uint32_t a1s_lo = (uint32_t)(a1s & 0xffffffffULL);
  const uint32_t a1s_hi = (uint32_t)((a1s >> 32) & 0xffffffffULL);
  const uint32_t a1e_lo = (uint32_t)(a1e & 0xffffffffULL);
  const uint32_t a1e_hi = (uint32_t)((a1e >> 32) & 0xffffffffULL);

  const int allow2_ok = (allow2_start != 0 && allow2_end_inclusive != 0 && allow2_end_inclusive >= allow2_start);
  const uint64_t a2s = allow2_ok ? (uint64_t)allow2_start : 0;
  const uint64_t a2e = allow2_ok ? (uint64_t)allow2_end_inclusive : 0;
  const uint32_t a2s_lo = (uint32_t)(a2s & 0xffffffffULL);
  const uint32_t a2s_hi = (uint32_t)((a2s >> 32) & 0xffffffffULL);
  const uint32_t a2e_lo = (uint32_t)(a2e & 0xffffffffULL);
  const uint32_t a2e_hi = (uint32_t)((a2e >> 32) & 0xffffffffULL);

  struct sock_filter filter[128];
  size_t n = 0;

  auto emit_stmt = [&](uint16_t code, uint32_t k) {
    filter[n++] = BPF_STMT(code, k);
  };
  auto emit_jump = [&](uint16_t code, uint32_t k, uint8_t jt, uint8_t jf) {
    filter[n++] = BPF_JUMP(code, k, jt, jf);
  };

  emit_stmt(BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, nr));

  unsigned int syscalls[7];
  int syscall_count = 0;
  const int full = __atomic_load_n(&g_seccomp_full_enabled, __ATOMIC_RELAXED) != 0;
  if (full) {
    syscalls[syscall_count++] = kSysOpenat;
    syscalls[syscall_count++] = kSysOpenat2;
    syscalls[syscall_count++] = kSysFaccessat;
    syscalls[syscall_count++] = kSysNewfstatat;
    syscalls[syscall_count++] = kSysStatx;
    syscalls[syscall_count++] = kSysReadlinkat;
  } else {
    syscalls[syscall_count++] = kSysOpenat;
    syscalls[syscall_count++] = kSysOpenat2;
    syscalls[syscall_count++] = kSysReadlinkat;
  ...
  if (allow2_ok) {
    syscalls[syscall_count++] = kSysClose;
  }

  size_t jeq_idx[7];
  for (int i = 0; i < syscall_count; ++i) {
    jeq_idx[i] = n;
    emit_jump(BPF_JMP | BPF_JEQ | BPF_K, syscalls[i], 0, 0);
  }
  emit_stmt(BPF_RET | BPF_K, SECCOMP_RET_ALLOW);

  size_t check_ip = n;
  for (int i = 0; i < syscall_count; ++i) {
    const size_t idx = jeq_idx[i];
    const size_t delta = check_ip - (idx + 1);
    filter[idx].jt = (uint8_t)delta;
    filter[idx].jf = 0;
  }

  auto emit_allow_range_to = [&](uint32_t start_hi,
                                 uint32_t start_lo,
                                 uint32_t end_hi,
                                 uint32_t end_lo,
                                 size_t* next_jt,
                                 ...
                                 int* next_jt_count,
                                 int* next_jf_count) {
    emit_stmt(BPF_LD | BPF_W | BPF_ABS, ip_off + 4);

    const size_t idx_ge_start = n;
    emit_jump(BPF_JMP | BPF_JGE | BPF_K, start_hi, 0, 0);
    next_jf[(*next_jf_count)++] = idx_ge_start;

    const size_t idx_gt_end = n;
    emit_jump(BPF_JMP | BPF_JGT | BPF_K, end_hi, 0, 0);
    next_jt[(*next_jt_count)++] = idx_gt_end;

    const size_t idx_eq_start = n;
    emit_jump(BPF_JMP | BPF_JEQ | BPF_K, start_hi, 0, 0);

    const size_t idx_eq_end = n;
    emit_jump(BPF_JMP | BPF_JEQ | BPF_K, end_hi, 0, 0);

    emit_stmt(BPF_RET | BPF_K, SECCOMP_RET_ALLOW);

    const size_t label_start_lo = n;
    emit_stmt(BPF_LD | BPF_W | BPF_ABS, ip_off + 0);
    const size_t idx_ge_start_lo = n;
    emit_jump(BPF_JMP | BPF_JGE | BPF_K, start_lo, 0, 0);
    next_jf[(*next_jf_count)++] = idx_ge_start_lo;
    emit_stmt(BPF_RET | BPF_K, SECCOMP_RET_ALLOW);

    const size_t label_end_lo = n;
    emit_stmt(BPF_LD | BPF_W | BPF_ABS, ip_off + 0);
    const size_t idx_gt_end_lo = n;
    emit_jump(BPF_JMP | BPF_JGT | BPF_K, end_lo, 0, 0);
    next_jt[(*next_jt_count)++] = idx_gt_end_lo;
    emit_stmt(BPF_RET | BPF_K, SECCOMP_RET_ALLOW);

    const size_t d1 = label_start_lo - (idx_eq_start + 1);
    const size_t d2 = label_end_lo - (idx_eq_end + 1);
    filter[idx_eq_start].jt = (uint8_t)d1;
    filter[idx_eq_start].jf = 0;
    filter[idx_eq_end].jt = (uint8_t)d2;
    filter[idx_eq_end].jf = 0;
  };

  size_t allow1_next_jt[8];
  size_t allow1_next_jf[8];
  int allow1_next_jt_count = 0;
  int allow1_next_jf_count = 0;
  emit_allow_range_to(a1s_hi, a1s_lo, a1e_hi, a1e_lo,
                      allow1_next_jt, allow1_next_jf,
                      &allow1_next_jt_count, &allow1_next_jf_count);

  size_t allow2_label = n;
  if (allow2_ok) {
    for (int i = 0; i < allow1_next_jt_count; ++i) {
      const size_t idx = allow1_next_jt[i];
      const size_t delta = allow2_label - (idx + 1);
      filter[idx].jt = (uint8_t)delta;
    }
    for (int i = 0; i < allow1_next_jf_count; ++i) {
      const size_t idx = allow1_next_jf[i];
      const size_t delta = allow2_label - (idx + 1);
      filter[idx].jf = (uint8_t)delta;
    }

    size_t allow2_next_jt[8];
    size_t allow2_next_jf[8];
    int allow2_next_jt_count = 0;
    int allow2_next_jf_count = 0;
    emit_allow_range_to(a2s_hi, a2s_lo, a2e_hi, a2e_lo,
                        allow2_next_jt, allow2_next_jf,
                        &allow2_next_jt_count, &allow2_next_jf_count);

    const size_t trap_label = n;
    for (int i = 0; i < allow2_next_jt_count; ++i) {
      const size_t idx = allow2_next_jt[i];
      const size_t delta = trap_label - (idx + 1);
      filter[idx].jt = (uint8_t)delta;
    }
    for (int i = 0; i < allow2_next_jf_count; ++i) {
      const size_t idx = allow2_next_jf[i];
      const size_t delta = trap_label - (idx + 1);
      filter[idx].jf = (uint8_t)delta;
    }
  } else {
...
# define SET_RET(v) ((void)(v))
   ...
   if (nr != (long)kSysOpenat
       ...
       && nr != (long)kSysStatx
       && nr != (long)kSysReadlinkat
       && nr != (long)kSysClose) return;
   ...
   if (nr == (long)kSysClose) {
       int fd = (int)a0;
       unmark_redirected_fd(fd);
       long ret = raw_syscall6((long)kSysClose, a0, 0, 0, 0, 0, 0);
       SET_RET(ret);
...
static void install_seccomp_backend_best_effort(int final_backend) {
  ...
  uintptr_t allow2_start = 0;
  uintptr_t allow2_end = 0;
  if (final_backend == REDIRECT_BACKEND_HYBRID) {
    if (find_libc_map_range(&g_libc_map_start, &g_libc_map_end_inclusive) == 0) {
      allow2_start = g_libc_map_start;
      allow2_end = g_libc_map_end_inclusive;
    } else {
      LOGE("find_libc_map_range failed");
    }
  }

  if (install_seccomp_filter_all_threads(g_self_map_start, g_self_map_end_inclusive, allow2_start, allow2_end) != 0) {
...
Java_com_xwaaa_hook_HookApplication_hookApkPathWithBackend(
  ...
  int b = (int)backend;
#if !defined(RESIG_HAS_BYTEHOOK) || !(RESIG_HAS_BYTEHOOK)
  if (b == REDIRECT_BACKEND_PLT || b == REDIRECT_BACKEND_HYBRID) {
      LOGE("bytehook not available, fallback to seccomp");
      b = REDIRECT_BACKEND_SECCOMP_SIGSYS;
  }
#endif
  if (b == REDIRECT_BACKEND_PLT) {
      install_plt_backend_best_effort();
